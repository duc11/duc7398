        ...
        <div class="product-image-wrapper">
            <div class="single-products">
                <div class="productinfo text-center">
                    <img src="/admin/images/product/${p.image}" height="200px" >
    ...
    <nav aria-label="Page navigation example <c:if test="${list.totalPages>0}"></c:if>">
        <ul class="pagination">
            <li class="page-item <c:if test="${1==list.number+1}">active</c:if>">
                <a class="page-link " href="/product/search?name=${name}&size=${list.size}&page=${1}">first</a></li>
            <c:forEach begin="1" step="1" end="${list.totalPages}" var="p">
                <li class="page-item  <c:if test="${p==list.number+1}">active</c:if>">
                    <a class="page-link " href="/product/search?name=${name}&size=${list.size}&page=${p}">${p}</a></li>
            </c:forEach>
            <li class="page-item <c:if test="${list.totalPages==list.number+1}">active</c:if>">
                <a class="page-link " href="/product/search?name=${name}&size=${list.size}&page=${list.totalPages}">last</a></li>
        </ul>
    </nav>
</div>
